...
interface StepAssociationModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (stepIds: number[]) => void;
    skillName: string;
    existingProjects: { id: number; name: string }[];
    existingSteps: { id: number; name: string; projectId: number }[];
    currentStepIds?: number[];
}
...
const StepAssociationModal: React.FC<StepAssociationModalProps> = ({
    ...
    const getHelperStepName = (stepId: number) => {
        return existingSteps.find((s) => s.id === stepId)?.name || "Inconnu";
    };
            ...
            <div className="w-full max-w-4xl rounded-xl bg-surface text-text-main shadow-2xl overflow-hidden flex flex-col border border-border">
                ...
                <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6">
                    ...
                    <div className="flex-1">
                        <div className="border border-border rounded-lg p-4 h-[400px] overflow-y-auto">
                            ...
                            <div className="space-y-2">
                                ...
                                {existingSteps.length === 0 && (
                                    <div className="text-text-muted italic">Aucune étape disponible.</div>
                                )}
                            </div>
                        </div>
                    </div>
